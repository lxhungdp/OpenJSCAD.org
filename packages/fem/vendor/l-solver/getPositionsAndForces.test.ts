...
import * as stiffnessSolveOps from "./stiffnessSolveOps";
...
describe("getPositionsAndForces", () => {
  ...
  test("sparse-global backend matches dense-legacy on cantilever column", () => {
    ...
    const loads: Mesh["loads"]["val"] = new Map([[1, [10, -2000, 0, 0, 0, 0]]]);
    const elementsProps: Mesh["elementsProps"]["val"] = new Map(
      elements.map((_, i) => [i, { ...genericMemberProps }]),
    );

    const backends: LinearSolverBackend[] = [
      "mathjs-dense-legacy",
      "mathjs-sparse-global",
    ];
    const results = backends.map((linearSolverBackend) =>
    ...
    expect(results[0]!.positions.length).toBe(results[1]!.positions.length);
    for (let i = 0; i < results[0]!.positions.length; i++) {
      expect(results[1]!.positions[i]).toBeCloseTo(results[0]!.positions[i], 7);
    }
  });
  ...
  test("3- 2-story frame: fixed supports with releases on beams", () => {
    ...
    const releases: Mesh["releases"]["val"] = new Map([
      [2, [true, true]], // both-ends on 1st floor beam (Mz-only)
      [5, [false, true]], // end release on roof beam (Mz-only at end)
    ]);
  ...
  test("reuses LUP cache when only nodal loads change (same mesh, supports, props)", () => {
    const lupSpy = vi.spyOn(stiffnessSolveOps, "stiffnessLup");
    ...
    const loads1: Mesh["loads"]["val"] = new Map([[1, [10, 0, 0, 0, 0, 0]]]);
    const first = getPositionsAndForces(
      nodes,
      elements,
      loads1,
      supports,
    ...
    expect(lupSpy).toHaveBeenCalledTimes(1);

    const loads2: Mesh["loads"]["val"] = new Map([[1, [20, 0, 0, 0, 0, 0]]]);
    lupSpy.mockClear();
    const reused = getPositionsAndForces(
      nodes,
      elements,
      loads2,
      supports,
      elementsProps,
      undefined,
      { cache: first.cache, linearSolverBackend: "mathjs-dense-legacy" },
    );
    expect(lupSpy).toHaveBeenCalledTimes(0);

    const fresh = getPositionsAndForces(
      nodes,
      elements,
      loads2,
      supports,
    ...
    expect(reused.positions.length).toBe(fresh.positions.length);
    for (let i = 0; i < fresh.positions.length; i++) {
      expect(reused.positions[i]).toBeCloseTo(fresh.positions[i], 8);
    }

    const supports2: Mesh["supports"]["val"] = new Map([
  ...
  test("fixed-fixed two-span beam: unequal EI redistributes bending moments vs equal EI", () => {
    const nodes: Mesh["nodes"]["val"] = [
      [0, 0, 0],
      [5, 0, 0],
      [10, 0, 0],
    ...
    const loads: Mesh["loads"]["val"] = new Map([[1, [0, -10, 0, 0, 0, 0]]]);

    const base = { ...genericMemberProps };
    const equalProps: Mesh["elementsProps"]["val"] = new Map([
      [0, { ...base }],
      [1, { ...base }],
    ]);
    const stiffLeftProps: Mesh["elementsProps"]["val"] = new Map([
      [0, { ...base, momentInertia: base.momentInertia * 80 }],
      [1, { ...base, momentInertia: base.momentInertia * 0.05 }],
    ]);

    const equalRun = getPositionsAndForces(
      nodes,
      elements,
      loads,
      supports,
    ...
    const stiffRun = getPositionsAndForces(
      nodes,
      elements,
      loads,
      supports,
    ...
    const mEq = equalRun.internalForces.get(0)!.Mz;
    const mSt = stiffRun.internalForces.get(0)!.Mz;
    const delta =
      Math.abs(mEq[0] - mSt[0]) +
      Math.abs(mEq[1] - mSt[1]) +
      Math.abs(
        equalRun.internalForces.get(1)!.Mz[0] - stiffRun.internalForces.get(1)!.Mz[0],
      );

    expect(delta).toBeGreaterThan(1e-3);
  });

  test("zero Iy/Iz and zero G/J with E>0 still yields a solvable system (property-edit guard)", () => {
    const nodes: Mesh["nodes"]["val"] = [
      [0, 0, 0],
      [0, 3, 0],
    ];
    ...
    const elementsProps: Mesh["elementsProps"]["val"] = new Map([
      [
        0,
        {
          elasticity: genericMemberProps.elasticity,
          area: genericMemberProps.area,
          momentInertiaY: 0,
          momentInertiaZ: 0,
          shearModulus: 0,
          torsionalConstant: 0,
    ...
    for (const linearSolverBackend of backends) {
